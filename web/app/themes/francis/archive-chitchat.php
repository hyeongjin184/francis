    ...
    <div id="primary" class="content-area">
        <main id="main" class="site-main" role="main">
            ...
            <div class="archive-background-container">
                <div class="archive-background archive-background-chitchat"></div>
                            ...
                            if (have_posts()): ?>
                                <ul class="article-list">
                                    ...
                                    while (have_posts()):the_post();
                                        $chitchatEpisodeImageUrl = "";
                                        if (has_post_thumbnail()) {
                                            // 아이캐치 이미지가 있을 경우
                                            $chitchatEpisodeImageUrl = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'article_thumb')[0];
                                        } else {
                                            // 아이캐치 이미지가 없을 경우
                                            $chitchatEpisodeImageUrl = get_stylesheet_directory_uri() . "/image/dummy/noimage.jpg";
                                        }
                                        ?>
                                        <li class="article-list__item">
                                            <a href="<?php the_permalink(); ?>">
                                                ...
                                                <div class="image-container">
                                                    <img src="<?= $chitchatEpisodeImageUrl; ?>" alt="<?php the_title(); ?>">
                                                </div>
                                                <p class="ariticle-content"><?= wp_trim_words(get_the_content(), 30, '⋯'); ?></p>
                                                <ul class="article-tags-container">
                                                <?php
                                                $articleTags = get_the_terms($post->ID, 'chitchat-tag');
                                                if ($articleTags) {
                        ...
                        <div class="archive-list-and-tags__tags-container">
                            <ul class="archive-list-and-tags__tags-container__tags">
                                ...
                                <li class="tag-list <?= $isTheTagSelected; ?>">
                                    <a href="<?= get_post_type_archive_link('chitchat'); ?>" class="tag-list__link">
                                        <p>#전체</p>
                                    </a>
                                </li>
                                <?php
                                ...
                                if ($allChitchatTags) {
                                    foreach ($allChitchatTags as $tag) :
                                        $linkToTagLimitedArchiveJapan = get_post_type_archive_link('chitchat') . '?chitchat-tag=' . $tag->slug;
                                        $isTheTagSelected = '';
